...
from web3_stuff.apeSwap_constants import (
    APESWAP_ROUTER_ADDR,
    APESWAP_ROUTER_ABI,
)
...
router_contract: Contract = PROVIDER.eth.contract(
    APESWAP_ROUTER_ADDR, abi=APESWAP_ROUTER_ABI
)


def run_ape(token_addr: ChecksumAddress):
    data = get_exchange_price_single_token(
        token_addr, router_contract, "ApeSwap.finance"
    )
    return data
